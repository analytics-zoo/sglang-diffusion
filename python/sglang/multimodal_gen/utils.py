...
def _patched_set_stream(stream: torch.xpu.Stream | None) -> None:
...
def current_stream() -> torch.xpu.Stream | None:
    """
    replace `torch.xpu.current_stream()` with `sglang.multimodal_gen.utils.current_stream()`.
